...
class Player:
    ...
    def __init__(self, player, x, y, flip, data, sprite_sheet, animation_steps, sound, control_mode="clavier", joystick_id=None):
        ...
        self.hit = False
        self.health = 100
        self.energy = 0  # Energy gauge from 0 to 100
        self.alive = True
        self.can_double_jump = True
        self.hit_counter = 0

        # Control mode: gamepad or keyboard
        ...
        if control_mode == "manette" and joystick_id is not None:
            if pygame.joystick.get_count() > joystick_id:
                ...
                self.joystick.init()
                print(f"🕹️ Controller {joystick_id} assigned to player {self.player}")
            else:
                print(f"⚠️ Controller {joystick_id} not detected for player {self.player}")
        else:
            print(f"⌨️ Player {self.player} is using the keyboard")

    def load_images(self, sprite_sheet, animation_steps):
        """Load and scale images from the sprite sheet."""
        animation_list = []
    ...
    def move(self, screen_width, screen_height, surface, target, round_over):
        """Handle character movement along x and y axes."""
        speed = 10
        gravity = 2
        dx = 0
        dy = 0
            ...
            if self.control_mode == "clavier":
                ...
                if self.player == 2:
                    ...
                    if key[pygame.K_UP] and not self.jump:
                        self.vel_y = -30
                        self.jump = True
                        self.can_double_jump = True  # if work
                    # DOUBLE JUMP
                ...
                # Main jump with button X
                if self.joystick.get_button(0) and not self.jump:
                    self.vel_y = -30
                    self.jump = True
                    self.can_double_jump = True

                # Double jump with button 12 (D-pad up)
    ...
    def get_projectile_origin(self):
        """Returns the visual center of the sprite (torso) as the projectile origin."""
        ...
        origin_y = draw_y + sprite_height // 2.4  # torso center
